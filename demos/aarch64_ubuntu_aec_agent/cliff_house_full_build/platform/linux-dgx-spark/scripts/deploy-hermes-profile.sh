...
PROFILE_NAME="${HERMES_PROFILE_NAME:-cliff-house-full-build-linux}"
MODEL_ID="${MODEL_ID:-}"
...
DML_STORE="$DML_ROOT/stores/cliff-house-full-build-linux"
...
for item in hermes projects skills system_prompts; do
  mkdir -p -- "$PROFILE_ROOT/$item"
  cp -a -- "$WORKSPACE/$item/." "$PROFILE_ROOT/$item/"
done
...
"$DML_ROOT/.venv-dml/bin/python" "$WORKSPACE/../memory/seed_dml.py" \
  ...
  --knowledge "$WORKSPACE/memory" --project-id project:cliff-house-full-build-linux
